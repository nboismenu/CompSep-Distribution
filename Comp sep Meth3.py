...
def plot_losses(graph, graph2 = None):
    def trace(g, ax):
        inc = 0
        for l in g:
            ax.semilogy(np.arange(len(l)) + inc, l)
            inc += len(l)
    if graph2 is None:
...
from_PS_list = []
for i in range(nb_synthèses):
    rd = np.random.uniform(-0.8,0.8,len(PS(x))+6)
    rd = lissage(rd)
...
d = np.std(d)*np.random.randn(np.shape(x)[0], np.shape(x)[1]) # From a WN
...
if nb_synthèses > nb_bruit:
    raise TypeError('Number of CompSep have to be lower than the number of noise map')

d = np.array([iPS(from_PS) for from_PS in from_PS_list])
loss_graph = []
...
for k in range(nb_synthèses):
    ...
    for i in range(3):
        ...
        eval_cnt = 0
        result = opt.minimize(objective_JM, d_c.ravel(), method='L-BFGS-B', jac=True, tol=None, options={"maxiter": 80, "gtol": 1e-14, "ftol": 1e-14, "maxcor": 20})
...
fig.tight_layout()
fig.savefig('_Syntheses_1_et_2_both_met')
...
plt.legend()
plt.savefig('_famille_PS_both_met')

np.save('_syntheses_both_met.npy', d)